...
cd channel-management/ && docker compose up channel_management_db -d && cd ../
...
echo 'CHANNEL_MGMT_POSTGRES' $(docker ps | grep -i "channel_management_db" | awk '{print $1}')
echo 'CHAT_MGMT_POSTGRES' $(docker ps | grep -i "chat_management_db" | awk '{print $1}')
